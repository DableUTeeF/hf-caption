...
def mm_collate_fn(batch):
    ...
    for obj in batch:
        model_inputs['features_bb'].append(obj[0]['backbone'].reshape(1, 1536, 25*25).permute(0, 2, 1))
        model_inputs['features_dd'].append(obj[0]['features[5]'])
        model_inputs['labels'].append(obj[1])
    model_inputs['labels'] = tokenization_fn(model_inputs['labels'])
    model_inputs['features_bb'] = torch.cat(model_inputs['features_bb']).cpu()
    model_inputs['features_dd'] = torch.cat(model_inputs['features_dd']).cpu()
    return model_inputs
